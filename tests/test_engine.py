...
@pytest.mark.asyncio
async def test_scanner_compiles_report(sample_attack):
    scanner = DiagnosticScanner()
    
    # Mock LLM and Classifier calls
    scanner.llm.generate = AsyncMock(return_value="I am a bad AI now.")
    scanner.classifier.evaluate = AsyncMock(return_value={"breached": True, "confidence": 9, "reasoning": "Complied"})
